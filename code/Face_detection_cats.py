...
class CatFaceDetector():
    ...
    def detect(self, image, scale_factor,
        ...
        #convert to cv2 (in case it is a PIL image)
        cv2_image = cv.cvtColor(np.array(image), cv.COLOR_RGB2BGR)


        bbox=self.objectCascade.detectMultiScale(cv2_image,
                                                scaleFactor=scale_factor,
                                                minNeighbors=min_neighbors,
                                                minSize=min_size)
        return bbox
def detect_cat_face(image_gray, file_name, scale_factor=1.15, min_neighbors=1, min_size=(30,30)):
    '''
...
#    image_gray = cv.cvtColor(image, cv.COLOR_BGR2GRAY)
    ...
    frontal_cascade_path = r"C:\Users\ravit\PycharmProject\cats\code" \
                           r"\cat_face_detection\haarcascade_frontalcatface_extended.xml"
